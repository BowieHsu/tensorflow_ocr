...
def bboxes_filter_overlap(labels, bboxes,xs, ys, threshold, scope=None, assign_negative = False):
    ...
    with tf.name_scope(scope, 'bboxes_filter', [labels, bboxes]):
        ...
        # bbox_mask = tf.logical_and(ws > 10, hs > 10)
        # labels = tf.boolean_mask(labels, bbox_mask)
        # bboxes = tf.boolean_mask(bboxes, bbox_mask)
        # scores = bboxes_intersection(tf.constant([0, 0, 1, 1], bboxes.dtype),bboxes)
        # xs = tf.boolean_mask(xs, bbox_mask);
        # ys = tf.boolean_mask(ys, bbox_mask);
