...
import cv2
from data_convert import read_tfrecord
...
img, label = read_tfrecord('./tfrecords/train.tfrecords')
img_batch, label_batch = tf.train.batch([img, label], batch_size= 10 )
...
x_tensor = tf.reshape(x, [-1, 256, 32, 1])
...
trans_tensor = tf.reshape(h_trans, [-1, 256, 32, 1])
...
for epoch_i in range(n_epochs):
    for iter_i in range(iter_per_epoch - 1):
        ...
        if iter_i % 10 == 0:
            ...
            writer.add_summary(summaries, epoch_i * iter_per_epoch + iter_i)
        sess.run(optimizer, feed_dict={
            x:val , y: l, keep_prob: 0.8})
